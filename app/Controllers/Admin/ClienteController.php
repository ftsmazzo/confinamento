...
class ClienteController extends ControllerAdmin
{
    ...
    public function __construct()
    {
        ...
        $this->view->addData([
            ...
            "page" => [
                "title" => "Clientes",
                "desc" => "Cadastre e gerencie seus clientes",
            ],
     ...
     *
     * A query faz left join em situacao para nao quebrar quando um
    ...
    public function index(): void
    {
        ...
        $this->view->addData([
            "breadcrumb" => [
                ...
                "Clientes" => ["url" => false, "current" => true],
            ],
            "page" => [
                "title" => "Clientes",
                "desc" => "Cadastre e gerencie seus clientes",
        ...
        $clientes = Cliente::leftJoin("cliente_situacao as s", "c.id_situacao", "=", "s.id")
            ->select("c.*", "s.descricao as situacao_descricao", "s.cor as situacao_cor")
            ->orderBy("c.razao")
            ->get();
        ...
        foreach ($clientes as $cliente) {
            // Os campos abaixo sao derivados somente para a listagem.
            // Nada disso volta para o banco; serve apenas para manter a view limpa.
            $cliente->hash = md5($cliente->id);
            $cliente->tipo_print = $cliente->pessoa === "J" ? "PJ" : "PF";
            $cliente->principal_print = trim((string) ($cliente->razao ?: $cliente->nome)) ?: "-";
            $cliente->secundario_print = trim((string) ($cliente->nome ?: "")) ?: "-";
            $cliente->documento_print = $this->formatDocumento($cliente->pessoa, $cliente->documento);
            $cliente->contato_print = trim((string) ($cliente->contato ?: $cliente->telefone ?: $cliente->whatsapp)) ?: "-";
     ...
     *
     * A view recebe listas auxiliares para montar select de situacao
    ...
    public function new(): void
    {
        ...
        $this->view->addData([
            "breadcrumb" => [
                ...
                "Novo Cliente" => ["url" => false, "current" => true],
            ],
            "page" => [
                "title" => "Novo Cliente",
                "desc" => "Preencha os dados básicos do cliente",
            ],
        ]);
    ...
    public function edit(Request $request): void
    {
        ...
        $this->view->addData([
            "breadcrumb" => [
                "Dashboard" => ["url" => $this->router->route("admin.home"), "current" => false],
                "Clientes" => ["url" => $this->router->route("admin.cliente.index"), "current" => false],
                "Editar Cliente" => ["url" => false, "current" => true],
            ],
            "page" => [
                "title" => "Editar Cliente",
                "desc" => "Atualize os dados do cliente",
            ],
        ]);
     ...
     * @return array{imported:int,skipped:int,issues:array,issues_count:int}
     */
    ...
    /**
     * Retorna a URL da planilha modelo do cadastro de clientes.
     */
    private function importTemplateUrl(): string
    {
        return asset("docs/modelos/modelo-importacao-clientes-a.xlsx");
    }
}
