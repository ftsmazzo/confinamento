(function ( $ ) {
    ...
    $.fn.dizuploader = function( options ) {
        ...
        // Opções Padrão
        var settings = $.extend({
            extensions : this.data("extensions") ? this.data("extensions").split(/[\s,]+/).map(ext => ext.toLowerCase()) : ["*"], // Extensões de arquivos permitidas
            maxFiles   : this.data("max-files") ? this.data("max-files") : false,
            maxSize   : this.data("max-siz") ? this.data("max-size") : false,
